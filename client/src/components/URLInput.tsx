...
export function URLInput({ url, setUrl, onShorten, isLoading = false }: URLInputProps) {
  ...
  return (
    <div className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-white p-2 pl-6 shadow-sm focus-within:ring-2 focus-within:ring-indigo-600/20 focus-within:border-indigo-600 transition-all">
      ...
      <button
        onClick={onShorten}
        disabled={isLoading}
        className="flex items-center gap-2 rounded-xl bg-indigo-600 px-8 py-3.5 text-sm font-bold text-white hover:bg-indigo-700 transition-all active:scale-[0.98] cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
      >
        {isLoading ? (
          <>
